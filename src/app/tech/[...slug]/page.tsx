...
import type { Authors, Tech } from "contentlayer/generated";
import { allAuthors, allTeches } from "contentlayer/generated";
...
export async function generateMetadata({
  ...
  const post = allTeches.find((p) => p.slug === slugJoined);
...
export default async function Page({ params }: { params: Params }) {
  ...
  const sortedCoreContents = allCoreContent(sortPosts(allTeches));
  ...
  const post = allTeches.find((p) => p.slug === slugJoined) as Tech;
  const authorList = post?.authors || ["default"];
  const authorDetails = authorList.map((author) => {
    const authorResults = allAuthors.find((p) => p.slug === author);
    return coreContent(authorResults as Authors);
